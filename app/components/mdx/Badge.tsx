export function Badge({ children, type = "default" }: { children: React.ReactNode; type?: "default" | "new" | "deprecated" | "beta" }) {
  const styles = {
    ...
    deprecated: "bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400 border-red-200 dark:border-red-500/20",
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider border ${styles[type]}`}>
      {children}
    </span>
  );
}