...
export function Snippet({ children }: { children: string }) {
  ...
  return (
    ...
    >
      <span>{children}</span>
      <span className={`grid transition-all duration-300 ease-out ${copied ? 'grid-rows-[1fr] grid-cols-[1fr] opacity-100 ml-1.5' : 'grid-rows-[1fr] grid-cols-[0fr] opacity-0 ml-0 group-hover:grid-cols-[1fr] group-hover:opacity-100 group-hover:ml-1.5'}`}>
