...
function IconRenderer({ name, isActive }: { name?: string; isActive?: boolean }) {
  if (!name) return null;
  const Icon = (LucideIcons as any)[name];
  if (!Icon) return <LucideIcons.FileText className={`w-4 h-4 mr-3 shrink-0 ${isActive ? 'text-blue-500' : ''}`} />;
  return <Icon className={`w-4 h-4 mr-3 shrink-0 transition-colors duration-300 ${isActive ? 'text-blue-500 dark:text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]' : ''}`} />;
}
            ...
            return (
              <li key={j} className="relative group/item">
                ...
                >
                  ...
                  <span className="relative flex-1 min-w-0 leading-snug transition-transform duration-300 group-hover/item:translate-x-1">{item.title}</span>
                </Link>
              </li>
            )
          })}
          ...
          return (
            <div key={i} className="mb-2 relative group/item">
              ...
              >
                ...
                <span className="relative flex-1 min-w-0 leading-snug transition-transform duration-300 group-hover/item:translate-x-1">{section.title}</span>
              </Link>
            </div>
          );
        })}
