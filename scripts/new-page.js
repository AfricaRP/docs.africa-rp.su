...
const ask = (query) =>
  new Promise((resolve) => rl.question(`\x1b[36m? \x1b[0m${query}`, resolve));
...
async function main() {
  ...
  if (categories.length > 0) {
    ...
    const catChoice = await ask(
      "В какую категорию добавить? (введите номер или новое название): ",
    );
    ...
    if (!isNaN(num) && num === 0) {
      ...
      const newCatDir = path.join(
        contentDir,
        catChoice.trim().toLowerCase().replace(/\s+/g, "-"),
      );
  ...
  const icon = await ask(
    "Введите название иконки (Lucide) или оставьте пустым (напр. FileText): ",
  );
